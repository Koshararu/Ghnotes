    ...
    async def ttdcmd(self, message):
        ...
        try:
            ...
            if media_messages:
                ...
                # Отправка фотографий
                if photos:
                    await message.client.send_file(message.to_id, photos, caption="Результат загрузки TikTok (фото)")

                # Отправка документов (видео/аудио)
                if documents:
                    for doc in documents:
                        await message.client.send_file(message.to_id, doc, caption="Результат загрузки TikTok")
        finally:
                ...
                for link in links:
                    ...
                    try:
                        ...
                        if media_messages:
                            ...
                            # Отправка фотографий
                            if photos:
                                await message.client.send_file(message.chat_id, photos, caption="Результат загрузки TikTok (фото)")

                            # Отправка документов (видео/аудио)
                            if documents:
                                for doc in documents:
                                    await message.client.send_file(message.chat_id, doc, caption="Результат загрузки TikTok")
                    finally:
                        # Удаляем обработчик событий и сообщения
                        message.client.remove_event_handler(handler, events.NewMessage(incoming=True, from_users=chat))
                        await bot_send_link.delete()
